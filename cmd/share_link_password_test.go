...
func TestRemoveSharedLinkPasswordSendsRequirePasswordFalse(t *testing.T) {
	...
	if err := dbx.RemoveSharedLinkPassword("https://example.com/link"); err != nil {
		t.Fatalf("RemoveSharedLinkPassword error: %v", err)
	}
	...
	requirePassword, ok := settings["require_password"].(bool)
	if !ok {
